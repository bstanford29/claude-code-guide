...
import { NAV_GROUPS, productLabel, tierColor } from '@/lib/nav';
import type { Product, Tier } from '@/lib/nav';
...
export function Sidebar({ isOpen, onClose }: { isOpen: boolean; onClose: () => void }) {
  ...
  return (
    <>
      ...
      >
        {isOpen && <NavContent pathname={pathname} onClose={onClose} />}
      </aside>

      <aside
        aria-label="Primary navigation"
        className="hidden md:block fixed z-40 top-[56px] left-0 bottom-0 w-[280px] overflow-y-auto bg-[#0b0b0b] border-r border-[var(--color-border)]"
      >
    ...
    <nav className="px-5 py-6 space-y-6">
      {NAV_GROUPS.map((group) => (
        <div key={`${group.product}-${group.title}`}>
          <GroupHeader product={group.product} tier={group.tier} title={group.title} kicker={group.kicker} />
  ...
  return (
    <div className="flex items-baseline justify-between gap-2 px-2">
      ...
      <span className="font-[family-name:var(--font-mono)] text-[9px] uppercase tracking-[0.1em] text-[#444]">
        {product === 'shared' ? kicker : productLabel(product)}
      </span>
    </div>
  );
}
